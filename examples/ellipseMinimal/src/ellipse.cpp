...
double lateral_accelerate;//x axis
double longitudinal_accelerate;//y axis
double down_accelerate;//z axis
...
SbgErrorCode onLogReceived(SbgEComHandle *pHandle, SbgEComClass msgClass, SbgEComMsgId msg, const SbgBinaryLogData *pLogData, void *pUserArg)
{
  ...
  long int WheelCount[4] = {0};//different from the class's implict data Odometry::WheelCount[4]
  ...
  switch (msg){
    ...
    case SBG_ECOM_LOG_IMU_DATA://看这里
      ...
      if(start_calc == true)
      {
        gyroscopes = pLogData->imuData.gyroscopes[2];//yaw angular velocity
        ...
        else if(cnt == 200)//what's the function of cnt?only occure the first circle?
        {
          gyro_shift /= 200;
          cnt++;

          std_msgs::Int64 msg_cnt;//ros's message passing
          msg_cnt.data = cnt;
          Time_info.publish(msg_cnt);
        }
        else
        {
          gyroscopes -= gyro_shift;//why substract this value?-null shift
          delta_angle = (last_gyroscopes + gyroscopes) * delta_time * 0.5;//longe-kuta integral mean filter,when it comes firstly,it may have a problem that'0+1'?
          last_gyroscopes = gyroscopes;
          angle_gyro -= delta_angle;//why not add?
        ...
        if(nn==0)//first
        {
            ...
            WheelDir1[2] = a3;
            WheelDir1[3] = a4;
            haha.GetWheelCount(WheelCount);//get Odometry::WheelCount for explict data Wheelcount
        }
        else if(nn == 1)//second(store twice for WheelDistance and WheelCount)
        {
          WheelDistance1[0] = o1;
          WheelDistance1[1] = o2;
          WheelDistance1[2] = o3;
        ...
        if(nn > 0)//second,third and so on
        {
            haha.GetOdometry(WheelDistance1, WheelDir1, state, delta_angle);//get Odometry::WheelDistance and Odometry::WheelDir simultaneously for explict data Wheelcount 
            li = haha.Update();//根据状态位选择模式
            ...
            if(mode_now == 0)//cumulate the delta x,delta y and transfrom to the world frame?//add delta value 
            {
              trace.x -= cos(trace.thea)*li.x-sin(trace.thea)*li.y;
              trace.y += cos(trace.thea)*li.y+sin(trace.thea)*li.x;
              trace.thea += li.thea;
              trace.thea = 0.02 * trace.thea + 0.98 * angle_gyro;//0.98?
            }
            else
            {
