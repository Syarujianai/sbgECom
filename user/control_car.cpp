...
#define PERIMETER 80.110613
//打开串口，初始化小车速度，建议输入0.2-0.5
car::car(float speed)
{
	// if((fd = serial.OpenPort(ttyUSB1)) < 0)
	if((fd = serial.OpenPort(ttyUSB0)) < 0)
        {
          //fprintf(stderr, "serial: Unable to get speed information.\n");
        }
    serial.SetPara(fd,6,8,1,0);
    velosity = speed;
}
//普通模式：向前移动的速度和角度
void car::Move(short int speed,int angle)
{
	...
	for(int i=0;i<cnt;i++)
		sum +=data_send[i];
	data_send[cnt++]=sum;
	serial.WriteData(fd,data_send,cnt);
}
//原地模式：绕原地来运动
...
//获取小车的速度
void car::GetInfo(int mo, int ang, float speed)
{
	mode = mo;
	if(speed <= 1.5 && speed > -1.5) velosity = speed;
	//printf("%f\n", velosity);
	angle = ang;
...
void car::DataReceive()
{
	int n = serial.ReadData(fd, data_receive, sizeof(data_receive)) / sizeof(unsigned char);
		...
		if ((short)data_receive[i] == 0xAA && (short)data_receive[i + 1] == 0xAA && (short)data_receive[i + 3] < 50)
		{
			...
			if ((len + 1) <= (n - i - 4))//长度足够的话
			{
				...
				if ((sum & 0x00FF) == (short)data_receive[i + len + 4])//校验通过
				{
					...
					if (func_word == 0x01)//func_word==0x01,represent the angle of wheel
					{
						sv1_ac_data = (int)((data_receive[i + 4] << 24) | (data_receive[i + 5] << 16) | (data_receive[i + 6] << 8) | data_receive[i + 7]);//joint the parts of data
						sv1_ang_data = (float)(sv1_ac_data * 360.0f / 51200.0f / 10.0f);//transform the data

						sv2_ac_data = (int)((data_receive[i + 8] << 24) | (data_receive[i + 9] << 16) | (data_receive[i + 10] << 8) | data_receive[i + 11]);
						sv2_ang_data = (float)(sv2_ac_data * 360.0f / 51200.0f / 10.0f);

						sv3_ac_data = (int)((data_receive[i + 12] << 24) | (data_receive[i + 13] << 16) | (data_receive[i + 14] << 8) | data_receive[i + 15]);
						sv3_ang_data = (float)(sv3_ac_data * 360.0f / 51200.0f / 10.0f);

						sv4_ac_data = (int)((data_receive[i + 16] << 24) | (data_receive[i + 17] << 16) | (data_receive[i + 18] << 8) | data_receive[i + 19]);
						sv4_ang_data = (float)(sv4_ac_data * 360.0f / 51200.0f / 10.0f);
					}
					else if (func_word == 0x02)
					{
						cur1_data = (short)((data_receive[i + 4] << 8) | data_receive[i + 5]) * 0.01f;//angle
					...
					else if (func_word == 0x03)//func_word==0x03,represent the distance of wheel
					{
